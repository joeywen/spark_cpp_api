...
jobject convertToJObject(JNIEnv *env, const std::map<int, std::map<int, std::vector<double> > >& hashmap) {
	...
	for (iter = hashmap.begin(); iter != hashmap.end(); ++iter) {
		...
		for (tmpiter = tmp.begin(); tmpiter != tmp.end(); ++ tmpiter) {
			...
			jobject v = convertToJObject<std::vector<double> >(env, value);
			assure(v != NULL, env);
...
std::string convertFromJObject(JNIEnv* env, jobject str) {
	...
	assure(mid != NULL, env);
 	jobject jstr = env->CallObjectMethod(str, mid);
 	assert(jstr != NULL);*/
...
std::map<int, std::vector<double> > convertFromJObject(JNIEnv* env,
	...
	while (env->CallBooleanMethod(jiterator, hasNext)) {
		// Map.Entry entry = iterator.next();vector
...
/**
 * convert Scala Tuple2<Integer, HashMap<Integer, Iterable<Double>>>
 * to CPP map<int, map<int, vector<double>>>
 */
template<>
std::map<int, std::map<int, std::vector<double> > > convertFromJObject(
		JNIEnv* env,
		jobject tuple/*Tuple2<Integer, HashMap<Integer, Iterable<Double>>>*/) {
	std::map<int, std::map<int, std::vector<double> > > result;

	jclass tuplecls = env->GetObjectClass(tuple);
	assert(tuplecls != NULL);
	...
	jobject integer = env->GetObjectField(tuple, _1);
	assert(integer != NULL);

	jobject hashmap = env->GetObjectField(tuple, _2);
	assert(hashmap != NULL);
	/////////////////////////////////
	...
	while (env->CallBooleanMethod(jiterator, hasNext)) {
		...
		std::vector<double> tmp = convertFromJObject<std::vector<double> >(env, jvalue);

		std::map<int, std::vector<double> > mm;

		result[convertFromJObject<int>(env, integer)] = mm;
	}
